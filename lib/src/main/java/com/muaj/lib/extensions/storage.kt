 ...
 * 应用程序目录("/data/data/<包名>")
 */
val Context.dataDirPath: String
//    get() = ContextCompat.getDataDir(this)?.absolutePath ?: ""
    get() = filesDir.parent
 ...
 * 获取应用外置文件目录
 *
 * 应用程序文件目录("/Android/data/<包名>/files/{type}")
 */
fun Context.externalFileDirPath(type: String): String {
    var file = getExternalFilesDir(type)
    var isExist = file.createOrExistsDir()
    return if (isExist) file.absolutePath else externalFileDirPath
}


/**
 * sd卡存在时，这种目录下的文件在应用被卸载时不会被删除
 *
 * @return
 */
fun Context.getExternalPrivateDirPath(name: String): String {
    val namedir = "/$name"
    var path: String
    if (isExternalStorageWritable) {
        path = Environment.getExternalStorageDirectory().absolutePath + namedir
        createOrExistsDir(path)
    } else path = this.externalFileDirPath(name)
//        else File(this.externalFileDirPath,name).absolutePath
    return path
}
 ...
 * 获取应用外置存放数据目录
 *
 * 应用程序目录("/Android/data/<包名>")
 */
val Context.externalDataDirPath: String
    get() = getExternalFilesDir("").parent
//    get() {
//        val path = externalFileDirPath
 ...
 *
 */
fun File.deleteDir(isContainRoot: Boolean = false): Boolean {
    ...
    return if (isContainRoot) delete() else true
}
 ...
 *
 * @param dirPath
 * @param isContainRoot 是否删除最外层的文件夹
 * @return true  false
 *
 */
fun deleteDir(dirPath: String, isContainRoot: Boolean = false): Boolean {
    val file = getFileByPath(dirPath)
    return file?.deleteDir(isContainRoot) ?: false
}
...
fun getCacheSize(context: Context): String {
    val len = getDirLength(context.cacheDirPath) + getDirLength(context.externalCacheDirPath)
    return if (len == -1L) "" else byte2FitMemorySize(len)
}
...
fun clearCache(context: Context) {
    deleteDir(context.cacheDirPath)
    deleteDir(context.externalCacheDirPath)
}
...
fun clearData(context: Context) {
    deleteDir(context.dataDirPath)
    deleteDir(context.externalDataDirPath)
}